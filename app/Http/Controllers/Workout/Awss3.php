        ...
        if ($validation->fails()) {
            ...
            if (!empty($currentUser)) {
                ...
                if (!empty($workoutQuery)) {
                    ...
                    $filename =  $utc_time->format('Ymd_His')."_".strtolower($biostripmac_id).'_'.$request->file_type.'.csv';
...
//
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getGenericSignedUrl(Request $request)
    {
        $validation = Validator::make($request->all(), [
            'filename' => 'required',
        ]);
        ...
        if ($validation->fails()) {
            ...
            if (!empty($currentUser)) {
                ...
                $command = $s3->getCommand('PutObject',
                                           [
                                               'Bucket' => AWS_S3_BUCKET,
                                               'Key' => AWS_S3_PREFIX_USERLOGS . '/' . $currentUser->id . '/' . $request->filename,
                                           ]
                );
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getBleGenericSignedUrl(Request $request)
    {
        $validation = Validator::make($request->all(), [
            'filename' => 'required',
        ]);
        ...
        if ($validation->fails()) {
            ...
            if (!empty($currentUser)) {
                ...
                $command = $s3->getCommand('PutObject',
                                           [
                                               'Bucket' => AWS_S3_BUCKET,
                                               'Key' => AWS_S3_PREFIX_USERBIN . '/' . $currentUser->id . '/' . $request->filename,
                                           ]
                );
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getMedicalGenericSignedUrl(Request $request)
    {
        $validation = Validator::make($request->all(), [
            'rec_name' => 'required',
            'rec_type' => 'required',
        ...
        if ($validation->fails()) {
            ...
            if (!empty($currentUser)) {
            $medicalFile = new FB_UserFiles();
            $medicalFile->fb_user_id = $request->user_id;
            $medicalFile->report_name = $request->rec_name;
            $medicalFile->report_type = $request->rec_type;
            $medicalFile->report_notes = $request->rec_notes;
            $medicalFile->save();
                //Create a S3Client
                $s3 = new S3Client([
                                       'version' => 'latest',
                ...
                $command = $s3->getCommand('PutObject',
                                           [
                                               'Bucket' => AWS_S3_BUCKET,
                                               'Key' => AWS_S3_PREFIX_USERMEDICAL . '/' . $currentUser->id . '/' . $request->rec_name,
                                           ]
                );
                
                try {
                    $req = $s3->createPresignedRequest($command, '+120 minutes');
    ...
    * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     */
   public function getAllBinFileSignedUrl(Request $request)
   {
        $validation = Validator::make($request->all(), [
            'filename' => 'required',
       ]);
           ...
           $currentUser = FB_User::where('id', $request->user_id)
            ...
            if (!empty($currentUser)) {
               ...
               $command = $s3->getCommand('PutObject',
                                           [
                                              'Bucket' => AWS_S3_BUCKET,
                                               'Key' => AWS_S3_PREFIX_USERALLBINFILES . '/' . $currentUser->id . '/' . $request->filename,
                                           ]
                );
