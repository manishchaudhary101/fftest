...
{
    public function getMedicalStats(Request $request)
    {
    ...
    if(!empty($currentUser))
        {
            $medical_array = FB_UserFiles::selectRaw('id,created_at,updated_at,deleted_at,report_name,report_type,report_notes,report_url')
                  ->where('fb_user_id',$currentUser->id)
            ...
            return response()->json(
                array(
                    ...
                    'data' => array(
                        'medical_array'=> $medical_array,               
                        ),
                    'message' => 'Medical Array',
                    'errors' => null,
     ...
     */
    public function createOrEditWorkout(Request $request)
    {
        ...
        if ($validation->fails()) {
            ...
            if (!empty($currentUser)) {
                ...
                foreach ($realWorkoutsDataArray as $WKData) {
                    ...
                    $startTime = \DateTime::createFromFormat('d-m-Y H:i:s', $WKData['startTime']);
                    $endTime = \DateTime::createFromFormat('d-m-Y H:i:s', $WKData['endTime']);
                    ...
                    if(!empty($startTime) && !empty($endTime) && $endTime > $startTime)
                    {
                        ...
                        {
                            $duplicateCheck = FB_Workout::where('biostrip_macid', '=', $request->biostrip_macid)
                                ->where('start_time', '=', $startTime)
                                ->where('created_by', '=', $currentUser->id)
                                ->where('status_enum', '=', ENUM_STATUS_ACTIVE)
                                ->first();
                        ...
                        //found an existing record, now check if the old and new title was sent
                        if(isset($WKData['old_title']))
                        {
                            if(!empty($duplicateCheck) && isset($WKData['title']))
                            {
//                                if($duplicateCheck->title == $WKData['old_title'])
//                                {
                                    $duplicateCheck->title = $WKData['title'];

                                    $duplicateCheck->modified_by = $currentUser->id;
                                    $duplicateCheck->updated_at = new \DateTime();
...
//                                }
                        ...
                        if ((empty($duplicateCheck) || isset($WKData['id'])) && !empty($WKData['derived_data'])) {

                            if (isset($WKData['id'])) {
                                $workout = FB_Workout::where('created_by', $currentUser->id)
                                    ->where('status_enum', ENUM_STATUS_ACTIVE)
                                    ->where('id', $WKData['id'])
                                    ->first();
                            ...
                            if (!empty($workout) && !isset($WKData['id']) ) {

                                $workout->title = $WKData['title'];
                                $workout->start_time = $startTime;
                                $workout->end_time = $endTime;

                                if (isset($WKData['time_zone']))
                                    $workout->time_zone = $WKData['time_zone'];
                                ...
                                $workout->biostrip_macid = $request->input('biostrip_macid');
                                $workout->firmware_version = $request->input('firmware_version');
                                ...
                                if (!empty($WKData['derived_data'])) {
                                    foreach ($WKData['derived_data'] as $data) {
                                        ...
                                        if (isset($data['dtime'])) {
                                            try {
                                                $dTime = \DateTime::createFromFormat('d-m-Y H:i:s', $data['dtime']);
            ...
            if (!empty($realWorkoutsDataArray)) {
                return response()->json(
                    array(
                        ...
                        'code' => RESPONSE_CODE_SUCCESS_OK,
                        'data' => null,
                        'message' => 'New Workouts Added:' . $totalWorkoutsAdded . ', Workouts Updated:' . $totalWorkoutsUpdated,
                        'errors' => null,
                    ), RESPONSE_CODE_SUCCESS_OK
                );
            } else {
    ...
    public function viewWorkout(Request $request)
    {
        ...
        if($validation->fails()){
            ...
            if (!empty($currentUser)) {
                ...
                select('id','created_at','created_by','status_enum','local_id','title','start_time','end_time','biostrip_macid','firmware_version','time_zone','activity_type')
                    ->where('created_by', $currentUser->id)
                    ->where('status_enum', ENUM_STATUS_ACTIVE);
                ...
                if ($request->filled('start_time')) {
                    ...
                    $startTime = \DateTime::createFromFormat('d-m-Y H:i:s', $request->start_time . ' 00:00:00');
                    if(!empty($startTime))
                    $workoutQuery->where('start_time', '>=', $startTime);
                }

                if ($request->filled('end_time')) {

                    $endTime = \DateTime::createFromFormat('d-m-Y H:i:s', $request->end_time . ' 23:59:59');
                    if(!empty($endTime))
                    $workoutQuery->where('end_time', '>=', $endTime);
                }
                ...
                array(
                    ...
                    'data' => array(
                        ...
                        'this_week' =>array(
                                        ...
                                        'end_date' => $end_date,
                                        'training_load' => intval($training_load),
    ...
    public function getHomeStats(Request $request)
    {
        ...
        if(!empty($currentUser))
        {
            ...
            $workoutStats = FB_Workout::selectRaw('count(distinct(start_time_utc)) as activities, ROUND(sum(distinct(time_to_sec(timediff(end_time,start_time ))) / 60)) as active_minutes, ROUND(sum(distinct(training_load))) as training_load')
                ->where('created_by',$currentUser->id)
                ->whereBetween('start_time',[$last_5_week_start_date,$end_date])
                ->distinct('start_time_utc')
                 //local 
...
//            $start_date = new \DateTime('last sunday 11:59:59PM'); //UTC
            ...
            $analysis = FB_Workout::selectRaw('FB_Insight.id,DATE(FB_Insight.created_at) AS created_at,DATE(FB_Insight.updated_at) AS updated_id,FB_Insight.created_by,FB_Insight.FC_Insight_category_id,FB_Insight.title,FB_Insight.content,FB_Workout.id')
                ...
                ->orderBy('FB_Workout.created_at', 'desc')
                ->get()->first();
...
//            $start_date = new \DateTime('last sunday 11:59:59PM'); //UTC
                ...
                $tdtl = FB_Workout::selectRaw('DATE(start_time_local) AS date, SUM(distinct(training_load)) AS points')
                ->where('created_by',$currentUser->id)
    ...
    foreach($workoutQuery->get() as $workout)
    {
        ...
        else
        {
            $returnarray[] = array(
                ...
                'newWorkoutType' => $workout->activity_type,
            );
        }
